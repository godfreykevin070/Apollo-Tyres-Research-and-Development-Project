...
const FTire: React.FC = () => {
  ...
  const validateParameters = (): boolean => {
    const required = ['p1', 'l1', 'l2', 'l3', 'vel', 'ia', 'sa', 'sr',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const storeResponse = await api.post("/store-ftire-data");
      ...
      // 6. Process project
      const processResponse = await api.post("/process-ftire", {
        projectId: Number(projectId),
        parameters,
      });
      ...
      if (processResponse.data.success) {
        ...
        setError(processResponse.data.message || "Failed to process FTire.");
      }
    } catch (error: any) {
      console.error(error);
  ...
  const parameterGroups = [
    {
      title: 'Pressure',
      fields: [
        { key: 'p1', label: 'P', placeholder: 'Enter pressure value in psi' },
      ]
    },
    {
      title: 'Load',
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-800">FTire Protocol</h1>
