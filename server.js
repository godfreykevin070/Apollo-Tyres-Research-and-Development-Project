...
const dbConfig = {
    ...
    password: process.env.DB_PASSWORD || 'tn22ct0524',
...
try {
    ...
    `, [
        ...
        'success',
        getClientIP(req),
        parseBrowser(req.headers['user-agent'] || '')
    ]);
} catch (logError) {
...
app.post('/api/store-cdtire-data', (req, res) => {
    ...
    db.query(truncateQuery, (truncateErr) => {
        ...
        Promise.all(insertPromises)
            ...
            // Add after the /api/store-cdtire-data success (around line 450):
...
.then(data => {
    ...
    fetch('/api/activity-log', {
        ...
        body: JSON.stringify({
            ...
            action: 'CDTire Data Processed',
            description: `Processed CDTire Excel data for project "${projectName}"`,
            status: 'success',
            metadata: {
...
// Replace the /api/generate-parameters endpoint (around line 1450):
...
app.post('/api/generate-parameters', (req, res) => {
    try {
        ...
        if (referer.includes('mf.html')) {
            templatePath = path.join(__dirname, 'templates', 'MF6pt2', 'parameters_template.inc');
        } else if (referer.includes('mf52.html')) {
            templatePath = path.join(__dirname, 'templates', 'MF5pt2', 'parameters_template.inc');
        } else if (referer.includes('ftire.html')) {
            templatePath = path.join(__dirname, 'templates', 'FTire', 'parameters_template.inc');
        } else if (referer.includes('cdtire.html')) {
            templatePath = path.join(__dirname, 'templates', 'CDTire', 'parameters_template.inc');
        } else if (referer.includes('custom.html')) {
            templatePath = path.join(__dirname, 'templates', 'Custom', 'parameters_template.inc');
        } else {
            return res.status(400).json({
                success: false,
                message: 'Unknown protocol page'
        ...
        //  Only replace parameters that have non-empty values
        const replacements = {
            '^load1_kg=': data.load1_kg || null,
            '^load2_kg=': data.load2_kg || null,
            '^load3_kg=': data.load3_kg || null,
            '^load4_kg=': data.load4_kg || null,
            '^load5_kg=': data.load5_kg || null,
            '^pressure1=': data.pressure1 || null,
            '^pressure2=': data.pressure2 || null,
            '^pressure3=': data.pressure3 || null,
            ...
            '^Outer_diameter=': data.Outer_diameter || null,
            '^nomwidth=': data.nomwidth || null,
            '^aspratio=': data.aspratio || null
        };

        console.log('📊 Generating parameters.inc with:', replacements);

        //  Replace each parameter if value is provided
        Object.entries(replacements).forEach(([key, value]) => {
            if (value !== null && value !== '') {
                const paramName = key.replace('^', '').replace('=', '');
                const regex = new RegExp(`^${paramName}\\s*=.*$`, 'gm');
                const replacement = `${paramName}=${value}`;
                
                if (regex.test(content)) {
                    content = content.replace(regex, replacement);
                    console.log(`✅ Replaced: ${paramName}=${value}`);
                }
            }
        });
        ...
        console.log('✅ parameters.inc written to:', outputPath);
        ...
        res.status(500).json({
            success: false,
            message: 'Error generating parameter file: ' + err.message
        });
    }
});
...
async function resolveDependencies(jobName, visitedJobs = new Set(), callerContext = null) {
    try {
        ...
        // --- FETCH per-row foltran / python_script CONTEXT (safe, no schema change) ---
        let perRowCtx = { foltranName: '', pythonScript: '' };
        try {
            // try direct select (if columns don't exist this will throw)
            const selQ = `SELECT foltran, python_script FROM ${tableName} WHERE job = $1 AND p = $2 AND l = $3 LIMIT 1`;
            const selRes = await db.query(selQ, [jobData.job, jobData.p, jobData.l]);
            if (selRes.rows && selRes.rows.length > 0) {
                const r = selRes.rows[0];
                perRowCtx.foltranName = r.foltran ? String(r.foltran).trim() : '';
                perRowCtx.pythonScript = r.python_script ? String(r.python_script).trim() : '';
            }
        } catch (ctxErr) {
            // If direct select failed (columns missing), attempt safe information_schema check
            try {
                ...
                `;
                const colCheckRes = await db.query(colCheckQ, [tableLower, ['foltran', 'python_script']]);
                const existingCols = (colCheckRes.rows || []).map(r => r.column_name);

                if (existingCols.includes('foltran') || existingCols.includes('python_script')) {
                    const extras = [];
                    if (existingCols.includes('foltran')) extras.push('foltran');
                    if (existingCols.includes('python_script')) extras.push('python_script');

                    const extraSelQ = `SELECT ${extras.join(', ')} FROM ${tableName} WHERE job = $1 AND p = $2 AND l = $3 LIMIT 1`;
                    const extraRes = await db.query(extraSelQ, [jobData.job, jobData.p, jobData.l]);
                    if (extraRes.rows && extraRes.rows.length > 0) {
                        const er = extraRes.rows[0];
                        if (er.foltran) perRowCtx.foltranName = String(er.foltran).trim();
                        if (er.python_script) perRowCtx.pythonScript = String(er.python_script).trim();
                    }
                } else {
                    // no optional columns present, leave ctx empty
                    console.log('No foltran/python_script columns present for per-row fetch.');
                }
            } catch (innerErr) {
                console.warn('Failed to fetch per-row foltran/python_script context:', innerErr.message);
            }
        }

        // set global context just for this execution, call executeAbaqusJob, then clean up
        global.__resolveJobContext = perRowCtx;
        let executeResult;
        try {
            executeResult = await executeAbaqusJob(folderPath, odbJobName, jobData.old_job, folderName);
        } finally {
            try { delete global.__resolveJobContext; } catch (_) {}
        }

        if (!executeResult || !executeResult.success) {
            throw new Error(`Failed to execute job ${odbJobName} in folder ${folderName}: ${executeResult && (executeResult.message || executeResult.error) || 'unknown error'}`);
        }
    ...
    // Function to execute Abaqus job with enhanced dependency handling and real-time progress
// ...existing code...
function executeAbaqusJob(folderPath, jobName, oldJobName, folderName = '') {
  return new Promise(async (resolve) => {
    try {
      const send = (payload) => {
        try { broadcastSse('run-status', Object.assign({ run: jobName, folder: folderName }, payload)); } catch(e){ console.warn('SSE send failed', e); }
      };

      send({ message: 'Starting Abaqus job', status: 'starting', progress: 0 });

      const jobInput = `${jobName}.inp`;
      send({ message: `Running Abaqus: job=${jobName} input=${jobInput}`, status: 'running', progress: 5 });

      const { spawn } = require('child_process');
      const child = spawn('abaqus', ['job=' + jobName, 'input=' + jobInput, 'interactive'], {
        cwd: folderPath,
        shell: false
      });

      registerChildProcess(child, { job: jobName, folder: folderName });

      child.stdout.on('data', d => send({ message: d.toString().trim().slice(0,300), status: 'running' }));
      child.stderr.on('data', d => send({ message: d.toString().trim().slice(0,300), status: 'running' }));

      let exitCode = await new Promise((res) => child.on('close', (c) => res(c)));

      if (exitCode !== 0) {
        send({ message: `Abaqus job exited with code ${exitCode}`, status: 'failed', progress: 100 });
        return resolve({ success: false, error: `Abaqus exited ${exitCode}` });
      }

      const fs = require('fs');
      const path = require('path');
      const odbPath = path.join(folderPath, `${jobName}.odb`);
      send({ message: 'Abaqus finished, verifying ODB', status: 'verifying', progress: 60 });

      if (!fs.existsSync(odbPath)) {
        send({ message: `ODB not found: ${odbPath}`, status: 'failed', progress: 100 });
        return resolve({ success: false, error: 'ODB not found after Abaqus run' });
      }

      send({ message: 'ODB found', status: 'completed', progress: 75 });

      // Read context set by caller (global as minimal-invasive approach)
      const pathTemplates = path.join(__dirname, 'templates', 'CDTire');
      const ctx = global.__resolveJobContext || {};
      const foltranName = (ctx.foltranName || '').trim();
      const pythonScript = (ctx.pythonScript || '').trim();

      // Prepare foltran if requested
      let foltranAvailable = false;
      let foltranProjectPath = '';
      const isValidName = s => s && s !== '-' && s.trim() !== '';

      if (isValidName(foltranName)) {
        const foltranFile = foltranName.endsWith('.f') ? foltranName : foltranName + '.f';
        const projectFoltran = path.join(folderPath, foltranFile);
        const templateFoltran = path.join(pathTemplates, foltranFile);

        if (fs.existsSync(projectFoltran)) {
          foltranAvailable = true;
          foltranProjectPath = projectFoltran;
          send({ message: `Using project foltran: ${foltranFile}`, status: 'info' });
        } else if (fs.existsSync(templateFoltran)) {
          try {
            fs.copyFileSync(templateFoltran, projectFoltran);
            foltranAvailable = true;
            foltranProjectPath = projectFoltran;
            send({ message: `Copied foltran from templates: ${foltranFile}`, status: 'info' });
          } catch (err) {
            send({ message: `Failed copying foltran: ${err.message}`, status: 'warning' });
          }
        } else {
          send({ message: `Foltran declared (${foltranFile}) but not found. Continuing without foltran.`, status: 'warning' });
        }
      }

      // Run python script if present
      if (isValidName(pythonScript)) {
        const pyFile = pythonScript.endsWith('.py') ? pythonScript : pythonScript + '.py';
        const templatePy = path.join(pathTemplates, pyFile);

        if (!fs.existsSync(templatePy)) {
          send({ message: `Python script not found in templates/CDTire: ${pyFile}. Skipping python step.`, status: 'warning' });
          return resolve({ success: true, job: jobName, odb: odbPath });
        }

        const argToPy = foltranAvailable ? path.basename(foltranProjectPath) : jobName;
        send({ message: `Running python script ${pyFile} with arg ${argToPy}`, status: 'python-start', progress: 80 });

        // Run using abaqus python to preserve Abaqus PYTHONPATH
        const pyChild = spawn('abaqus', ['python', pyFile, argToPy], {
          cwd: pathTemplates,
          shell: false
        });

        registerChildProcess(pyChild, { job: jobName, script: pyFile, folder: folderName });

        pyChild.stdout.on('data', d => send({ message: d.toString().trim().slice(0,300), status: 'python-running' }));
        pyChild.stderr.on('data', d => send({ message: d.toString().trim().slice(0,300), status: 'python-running' }));

        let pyExit = await new Promise((res) => pyChild.on('close', (c) => res(c)));

        if (pyExit !== 0) {
          send({ message: `Python script exited with code ${pyExit}`, status: 'python-failed', progress: 100 });
          return resolve({ success: false, error: `Python script failed (code ${pyExit})` });
        }

        send({ message: `Python script completed: ${pyFile}`, status: 'python-done', progress: 98 });
        return resolve({ success: true, job: jobName, odb: odbPath, python: pyFile, pythonArg: argToPy });
      }

      send({ message: 'No python script to run. Done.', status: 'done', progress: 100 });
      return resolve({ success: true, job: jobName, odb: odbPath });

    } catch (err) {
      console.error('executeAbaqusJob error', err);
      try { broadcastSse('run-status', { run: jobName, folder: folderName, message: 'Internal server error: ' + err.message, status: 'failed' }); } catch(e){}
      return resolve({ success: false, error: err.message || String(err) });
    }
  });
}
...
(async () => {
    try {
        // Get the initial job data (basic fields)
        ...
        // Attempt to read optional columns (foltran, python_script) only if they exist
        let foltranName = '';
        let pythonScript = '';
        ...
        try {
            ...
            if (existingCols.includes('foltran') || existingCols.includes('python_script')) {
                // Build select with only the columns that exist
                const extras = [];
                if (existingCols.includes('foltran')) extras.push('foltran');
                if (existingCols.includes('python_script')) extras.push('python_script');

                const extraSelQ = `SELECT ${extras.join(', ')} FROM ${tableName} WHERE number_of_runs = $1 LIMIT 1`;
                const extraRes = await db.query(extraSelQ, [runNumber]);
                if (extraRes.rows.length > 0) {
                    const extraRow = extraRes.rows[0];
                    if (extraRow.foltran) foltranName = String(extraRow.foltran).trim();
                    if (extraRow.python_script) pythonScript = String(extraRow.python_script).trim();
                }
            } else {
                console.log('Optional columns foltran / python_script not present in table; skipping extra context fetch.');
            }
        } catch (colErr) {
            console.warn('Could not determine optional columns or fetch their values:', colErr.message);
            // continue without optional context
        }

        // Prepare global context for executeAbaqusJob (so it can pick foltran/python names)
        global.__resolveJobContext = {
            foltranName: foltranName || '',
...
app.get('/api/projects/:id', authenticateToken, async (req, res) => {  
  try {
    ...
    const r = await db.query(q, [projectId]);
    
    if (!r.rows || r.rows.length === 0) {
      return res.status(404).json({ success: false, message: 'Project not found' });
    }
    
    //  Authorization check: engineers can only see their own projects
    if (req.user.role !== 'manager' && r.rows[0].user_email !== req.user.email) {
...
app.post('/api/check-project-exists', authenticateToken, async (req, res) => { 
  try {
    const { projectName, protocol } = req.body;
    ...
    const query = `
      SELECT id, project_name, protocol, status, user_email
      FROM projects 
      WHERE project_name = $1
      ORDER BY created_at DESC
      LIMIT 1
    ...
    if (result.rows.length > 0) {
      ...
      const folderName = `${projectName}_${protocol || project.protocol}`;
      ...
      return res.json({
        ...
        exists: false,
        project: null,
        folderName: `${projectName}_${protocol}`
      });
    }
    ...
    return res.status(500).json({ 
      success: false, 
      message: 'Server error checking project',
      error: err.message 
    });
  }
});
...
app.get('/api/projects/:projectId/matrix', authenticateToken, async (req, res) => {  
  try {
    const { projectId } = req.params;

    // Get project protocol
    const projectQuery = 'SELECT protocol, user_email FROM projects WHERE id = $1';
