...
const FTire: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await api.post('/process-ftire', {
        projectId,
        parameters,
        protocol: 'FTire'
      });
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Failed to process FTire data');
      }
    } catch (error: any) {
      console.error('Error submitting FTire:', error);
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <TestSummary protocol="FTire" projectId={projectId} />
          </div>
        </div>
      </main>
    </div>
